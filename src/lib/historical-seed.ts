...
export async function seedHistoricalData(db: Client): Promise<void> {
  ...
  function addTimeline(
    ...
    statements.push({
      ...
      args: [id, projectId, title, description, color, parentTimelineId, branchPointNodeId, orderIndex]
    });
  }
  ...
  function addDep(id: string, fromNodeId: string, toNodeId: string, type: string) {
    statements.push({
      sql: 'INSERT OR REPLACE INTO dependencies (id, from_node_id, to_node_id, type) VALUES (?, ?, ?, ?)',
      args: [id, fromNodeId, toNodeId, type]
    });
  }
...
addDep('dep-vn-th-regional', 'node-ly-thuong-kiet', 'node-th-lopburi-khmer', 'regional_balance');
  ...
  // Batch insert into db in chunks of 50
